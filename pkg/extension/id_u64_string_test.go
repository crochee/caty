...
func Test_Encode(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want:    []byte(`{"id":"787446465166","pid":0,"PartID":""}`),
			wantErr: false,
		},
	}
...
func Test_Decode(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "standard",
			input:   []byte(`{"id":"0","pid":0,"PartID":""}`),
			want:    TestString{},
			wantErr: false,
		},
		{
			name:    "none",
			input:   []byte(`{"id":"","pid":0,"PartID":""}`),
			want:    TestString{},
			wantErr: false,
		},
		{
			name:  "none",
			input: []byte(`{"id":"787446465166","pid":0,"PartID":""}`),
			want: TestString{
				ID: 787446465166,
			},
			wantErr: false,
		...
		{
			name:  "none",
			input: []byte(`{"id":"787446465166","pid":0,"PartID":"0"}`),
			want: TestString{
				ID: 787446465166,
			},
			wantErr: false,
