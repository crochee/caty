...
import (
	...
	"log"
	"obs/pkg/logx"
	rabbitmq2 "obs/pkg/message/rabbitmq"
	"time"
...
func Setup(ctx context.Context) error {
	...
	//	return err
	//}
	conn, err := rabbitmq2.NewMq(func(option *rabbitmq2.Option) {
		option.AmqpURI = "amqp://admin:1234567@192.168.31.62:5672/"
	}, func(option *rabbitmq2.Option) {
		option.Logger = log.NewLogger()
	})
	if err != nil {
		return err
	}
	...
	//pubSub := gochannel.NewGoChannel(gochannel.Config{}, logx)
	...
	//	return err
	//}
	subscriber := &rabbitmq2.Consumer{
		RabbitMq:    conn,
		ConsumeName: "-0",
	}
	subscriber1 := &rabbitmq2.Consumer{
		RabbitMq:    conn,
		ConsumeName: "-1",
	}
	...
	//		logx.Println("executing handler specific middleware for ", message.UUID)
	//		return h(message)
	//	}
	//})
...
func printMessages(ctx context.Context, subscriber rabbitmq2.Consume) {
